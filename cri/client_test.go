...
func TestUnixCRIUsesTheRuntimeSocketProtocol(t *testing.T) {
	...
	runtime := NewUnixCRI(listener.Addr().String())
	ctx := context.Background()
	if _, err := runtime.Run(ctx, api.Pod{ObjectMeta: api.ObjectMeta{Name: "nginx"}, Spec: api.PodSpec{Containers: []api.Container{{Image: "nginx"}}}}); err != nil {
		t.Fatal(err)
	}
	...
	if container, err := runtime.Inspect(ctx, "nginx"); err != nil || container.ID != "pid-1" {
		t.Fatalf("Inspect should return the selected container: %#v, %v", container, err)
	}
	if err := runtime.Stop(ctx, "nginx"); err != nil {
		t.Fatal(err)
	}
	<-done
}
